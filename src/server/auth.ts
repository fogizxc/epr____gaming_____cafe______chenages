...
export async function rotateRefreshToken(raw: string) {
  const tokenHash = crypto.createHash("sha256").update(raw).digest("hex");
  const collection = getDb().collection("refresh_tokens");
  ...
  const user = await getDb().collection("users").findOne({ _id: record.userId, isActive: { $ne: false } });
  if (!user) return null;
...
export async function revokeRefreshToken(raw: string) {
  const tokenHash = crypto.createHash("sha256").update(raw).digest("hex");
  await getDb().collection("refresh_tokens").updateOne({ tokenHash }, { $set: { revokedAt: new Date() } });
}
...
export function optionalAuth(req: AuthenticatedRequest, _res: Response, next: NextFunction) {
  ...
  if (token) {
    ...
    if (payload?.typ === "access" && payload.sub && payload.role) {
      req.user = { id: String(payload.sub), email: payload.email, name: payload.name, role: payload.role, permissions: payload.permissions || [] };
    }
  }
  next();
}
