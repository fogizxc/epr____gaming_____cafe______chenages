...
export interface AuthenticatedRequest extends Request {
  user?: { id: string; email?: string; name?: string; role: AppRole; permissions: string[] };
}
...
function verify(token: string): Record<string, any> | null {
  ...
  try {
    ...
    if (payload.typ !== "access" || typeof payload.exp !== "number" || payload.exp <= Math.floor(Date.now() / 1000)) return null;
    return payload;
  } catch { return null; }
}
...
export async function createRefreshToken(userId: string) {
  ...
  await db.collection("refresh_tokens").insertOne({ userId, tokenHash, createdAt: now, expiresAt: new Date(now.getTime() + REFRESH_TTL_SECONDS * 1000), revokedAt: null });
  return raw;
}
...
export async function rotateRefreshToken(raw: string) {
  ...
  const query = ObjectId.isValid(id)
    ...
    : { legacyId: id, isActive: { $ne: false } };
  const user = await db.collection("users").findOne(query as any);
  if (!user) return null;

  const permissions = Array.isArray(user.permissions) ? user.permissions : [];
  const accessToken = createAccessToken({ id: String(user._id), email: user.email, name: user.name, role: user.role as AppRole, permissions });
...
export async function revokeRefreshToken(raw: string) {
  const db = await getMongoDb();
  const tokenHash = crypto.createHash("sha256").update(raw).digest("hex");
  await db.collection("refresh_tokens").updateOne({ tokenHash, revokedAt: null }, { $set: { revokedAt: new Date() } });
}
