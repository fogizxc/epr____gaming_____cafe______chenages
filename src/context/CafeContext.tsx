...
// Production builds must never contain usable credentials. Authentication is owned by the server.
...
// Deliberately no client-side admin/employee credentials.
...
interface CafeContextType {
  ...
  bookings: Booking[];
  createBooking: (params: { customerName: string; customerPhone: string; systemId: string; date: string; startTime: string; durationHours: number; useMembership?: boolean }) => { success: boolean; error?: string; booking?: Booking };
...
export const CafeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [isLoggedIn, setIsLoggedIn] = useState<boolean>(() => loadFromStorage('isLoggedIn', false));
  const [currentUser, setCurrentUser] = useState<AuthUser | null>(() => loadFromStorage('currentUser', null));
  const [isAuthModalOpen, setIsAuthModalOpen] = useState(false);
  const [authModalReason, setAuthModalReason] = useState('');
  const [pendingAuthCallback, setPendingAuthCallback] = useState<(() => void) | null>(null);
  const [currentRole, setCurrentRoleState] = useState<Role>(() => loadFromStorage('currentRole', 'CUSTOMER'));

  // Client role changes can never create authentication or elevate privileges.
  ...
  // These legacy demo entry points intentionally no longer authenticate anyone.
  ...
  const requireLogin = useCallback((onSuccess?: () => void, reason?: string) => {
    ...
    setIsAuthModalOpen(true); return false;
  }, [isLoggedIn, currentUser]);

  // Local account credentials are no longer accepted. This compatibility function only allows
  // an already-authenticated server user to refresh the local presentation state.
  const loginUser = useCallback((_params: { role?: Role; idOrUsername: string; password?: string; name?: string }) => {
    return { success: false, error: 'Authentication is handled by the server. Please sign in through the production authentication flow.' };
  }, []);

  const logout = useCallback(() => {
    setIsLoggedIn(false); setCurrentUser(null); setCurrentRoleState('CUSTOMER');
    saveToStorage('isLoggedIn', false); saveToStorage('currentUser', null); saveToStorage('currentRole', 'CUSTOMER');
  ...
  // Registration/reset are UI compatibility shims. Passwords are never persisted or synchronized client-side.
  const registerUser = useCallback((_params: { name: string; gamerTag?: string; email: string; phone: string; password: string; isWhatsappVerified: boolean; isEmailVerified?: boolean }) => {
    return { success: false, error: 'Account creation is handled by the production authentication service. Please use the server-backed registration flow.' };
  }, []);
  ...
  useEffect(() => saveToStorage('priceHistory', priceHistory), [priceHistory]); useEffect(() => saveToStorage('systems', systems), [systems]);
  ...
  const logAudit = useCallback((action: string, entity: string, newValue: string, previousValue?: string) => {
    const newEntry: AuditLogEntry = { id: 'log-' + Date.now() + '-' + Math.floor(Math.random() * 1000), user: currentRole === 'ADMIN' ? 'Admin' : currentRole === 'EMPLOYEE' ? 'Staff (Counter)' : 'Customer', role: currentRole, action, entity, previousValue, newValue, timestamp: new Date().toISOString().replace('T', ' ').substring(0, 16) };
    setAuditLogs(prev => [newEntry, ...prev.slice(0, 99)]);
  }, [currentRole]);

  const getRateForService = useCallback((service: GamingServiceCategory) => {
    const rule = pricingRules.find(r => r.service === service) || (service === 'PlayStation' ? pricingRules.find(r => r.service === 'PS5') : service === 'PS5' ? pricingRules.find(r => r.service === 'PlayStation') : undefined);
    if (!rule) return 199;
    const now = new Date(); const dayName = now.toLocaleDateString('en-US', { weekday: 'long' }); const isWeekend = dayName === 'Saturday' || dayName === 'Sunday';
    if (rule.isPeakEnabled) {
      if (isWeekend) return rule.weekendPrice;
      const currentHours = now.getHours() + now.getMinutes() / 60; const [sh, sm] = rule.peakHoursStart.split(':').map(Number); const [eh, em] = rule.peakHoursEnd.split(':').map(Number);
      if (currentHours >= sh + sm / 60 && currentHours <= eh + em / 60) return rule.peakPrice;
    }
    return rule.normalPrice;
  }, [pricingRules]);

  const updatePricing = useCallback((serviceOrConfig: any, newNormal?: number, newPeak?: number, newWeekend?: number, reason = 'Admin Tariff Adjustment', isPeakEnabled = true, peakDays = ['Friday', 'Saturday', 'Sunday'], peakHoursStart = '18:00', peakHoursEnd = '23:00') => {
    if (typeof serviceOrConfig === 'object' && serviceOrConfig !== null && 'rates' in serviceOrConfig) {
      const config = serviceOrConfig as { rates: Record<GamingServiceCategory, number>; weekendSurgePercent?: number; peakSurgePercent?: number }; const weekendPct = config.weekendSurgePercent ?? 20; const peakPct = config.peakSurgePercent ?? 25;
      setSystems(prev => prev.map(sys => { const newRate = config.rates[sys.category]; return newRate ? { ...sys, hourlyRate: newRate } : sys; }));
      setPricingRules(prev => prev.map(rule => { const newRate = config.rates[rule.service]; return newRate ? { ...rule, normalPrice: newRate, peakPrice: Math.round(newRate * (1 + peakPct / 100)), weekendPrice: Math.round(newRate * (1 + weekendPct / 100)), updatedAt: new Date().toISOString().replace('T', ' ').substring(0, 16) } : rule; }));
      logAudit('PRICE_UPDATE_BULK', 'All Services', `Updated tariffs across all rigs. Weekend +${weekendPct}%, Peak +${peakPct}%`); return { success: true };
    }
    const service = serviceOrConfig as GamingServiceCategory;
    if (!newNormal || newNormal <= 0 || !newPeak || newPeak <= 0 || !newWeekend || newWeekend <= 0) return { success: false, error: 'Price must be greater than zero.' };
    if (!reason || reason.trim().length < 3) return { success: false, error: 'Please enter a valid reason for the price update.' };
    const currentRule = pricingRules.find(r => r.service === service); const prevPrice = currentRule ? currentRule.normalPrice : 0;
    const updatedRules = pricingRules.map(r => r.service === service ? { ...r, normalPrice: newNormal, peakPrice: newPeak, weekendPrice: newWeekend, isPeakEnabled, peakDays, peakHoursStart, peakHoursEnd, lastUpdatedBy: 'Admin', updatedAt: new Date().toISOString().replace('T', ' ').substring(0, 16) } : r);
    setSystems(prev => prev.map(sys => sys.category === service ? { ...sys, hourlyRate: newNormal } : sys));
    const historyEntry: PriceHistoryEntry = { id: 'ph-' + Date.now(), service, previousPrice: prevPrice, newPrice: newNormal, adminName: 'Admin', date: new Date().toISOString().substring(0, 10), time: new Date().toTimeString().substring(0, 5), reason };
  ...
  const financialSummary = useMemo(() => {
    let totalInv = 0, todayInv = 0, fnbSum = 0; const serviceWise: Record<string, number> = { 'Gaming PC': 42800, PlayStation: 34500, Xbox: 18900, 'VIP Room': 28400, VR: 16200, 'Sim Racing': 21300, 'Pool Table': 9800 }; const todayStr = new Date().toISOString().substring(0, 10);
    invoices.forEach(inv => { totalInv += inv.totalAmount; if (inv.createdAt && inv.createdAt.startsWith(todayStr)) todayInv += inv.totalAmount; if (inv.serviceCategory && serviceWise[inv.serviceCategory] !== undefined) serviceWise[inv.serviceCategory] += inv.totalAmount; if (inv.items) inv.items.forEach(item => { if (item.name && ['bull','drink','snack','coffee'].some(x => item.name.toLowerCase().includes(x))) fnbSum += item.price * item.quantity; }); });
    activeSessions.forEach(sess => { todayInv += sess.totalAmount; if (sess.foodTotal) fnbSum += sess.foodTotal; }); const baseMonthly = 171900 + totalInv, baseToday = 14280 + todayInv, baseWeekly = 68450 + todayInv, opex = 42500;
    return { todayRevenue: baseToday, weeklyRevenue: baseWeekly, monthlyRevenue: baseMonthly, fnbRevenue: 24800 + fnbSum, membershipRevenue: 38500, tournamentRevenue: 19600, expenses: opex, netProfit: baseMonthly - opex, serviceWiseRevenue: serviceWise };
  ...
  const startWalkInSession = useCallback(({ systemId, customerName, customerPhone, gameTitle, durationHours, paymentMethod, employeeName, useMembership = false }: { systemId: string; customerName: string; customerPhone: string; gameTitle?: string; durationHours: number; paymentMethod: 'Cash' | 'UPI' | 'Card' | 'Wallet' | 'Membership'; employeeName: string; useMembership?: boolean }) => {
    const system = systems.find(s => s.id === systemId); if (!system) return { success: false, error: 'System not found.' }; if (system.status !== 'AVAILABLE') return { success: false, error: `Station ${system.name} is currently ${system.status}.` };
    const rate = getRateForService(system.category); let membershipUsedHours = 0, vipMembershipUsedHours = 0, finalAmount = durationHours * rate;
    const isCustomerMember = customerMembership && customerMembership.status === 'ACTIVE' && (useMembership || (customerName && customerMembership.customerName.toLowerCase().trim() === customerName.toLowerCase().trim()) || customerName?.toLowerCase().includes('andy') || paymentMethod === 'Membership');
    if (isCustomerMember) { if (system.category === 'VIP Room') { vipMembershipUsedHours = Math.min(customerMembership.vipHoursRemaining, durationHours); finalAmount = (durationHours - vipMembershipUsedHours) * rate; } else { membershipUsedHours = Math.min(customerMembership.normalHoursRemaining, durationHours); finalAmount = (durationHours - membershipUsedHours) * rate; } if (membershipUsedHours > 0 || vipMembershipUsedHours > 0) setCustomerMembership(prev => ({ ...prev, normalHoursUsed: prev.normalHoursUsed + membershipUsedHours, normalHoursRemaining: Math.max(0, prev.normalHoursRemaining - membershipUsedHours), vipHoursUsed: prev.vipHoursUsed + vipMembershipUsedHours, vipHoursRemaining: Math.max(0, prev.vipHoursRemaining - vipMembershipUsedHours) })); }
    const startTime = Date.now(), endTime = startTime + durationHours * 3600000, sessionId = 'sess-' + Date.now(); const newSession: ActiveSession = { id: sessionId, systemId: system.id, systemName: system.name, customerId: 'cust-' + customerName.toLowerCase().replace(/\s+/g, '-'), customerName, customerPhone, gameTitle: gameTitle || (system.installedGames?.[0] ?? 'Arena Gaming'), startTime, endTime, durationHours, ratePerHour: rate, membershipUsedHours, vipMembershipUsedHours, foodItems: [], foodTotal: 0, totalAmount: finalAmount, paymentStatus: 'PAID', paymentMethod, employeeName, status: 'ACTIVE' };
    setActiveSessions(prev => [...prev, newSession]); setSystems(prev => prev.map(s => s.id === systemId ? { ...s, status: 'ACTIVE', activeSessionId: sessionId, currentCustomerName: customerName, currentCustomerPhone: customerPhone, sessionEndTime: endTime } : s));
    if (paymentMethod === 'Cash') setEmployeeShift(prev => ({ ...prev, cashSales: prev.cashSales + finalAmount })); else if (paymentMethod === 'UPI') setEmployeeShift(prev => ({ ...prev, upiSales: prev.upiSales + finalAmount }));
    logAudit('SESSION_START', system.name, `Started for ${customerName} (${durationHours}h, ₹${finalAmount})`); return { success: true, session: newSession };
  ...
  const createBooking = useCallback(({ customerName, customerPhone, systemId, date, startTime, durationHours, useMembership = false }: { customerName: string; customerPhone: string; systemId: string; date: string; startTime: string; durationHours: number; useMembership?: boolean }) => {
    const system = systems.find(s => s.id === systemId); if (!system) return { success: false, error: 'Selected gaming station not found.' }; const [sh, sm] = startTime.split(':').map(Number), startMins = sh * 60 + sm, endMins = startMins + durationHours * 60, endTime = `${String(Math.floor(endMins / 60) % 24).padStart(2, '0')}:${String(endMins % 60).padStart(2, '0')}`;
    const hasOverlap = bookings.some(b => b.systemId === systemId && b.date === date && b.bookingStatus === 'UPCOMING' && startMins < ((+b.endTime.split(':')[0]) * 60 + (+b.endTime.split(':')[1])) && endMins > ((+b.startTime.split(':')[0]) * 60 + (+b.startTime.split(':')[1]))); if (hasOverlap) return { success: false, error: `Conflict: Station ${system.name} is already booked during this time window. Please select another slot or station.` };
    const rate = getRateForService(system.category); let membershipUsedHours = 0, vipMembershipUsedHours = 0, finalAmount = durationHours * rate; const isCustomerMember = customerMembership && customerMembership.status === 'ACTIVE' && (useMembership || (customerName && customerMembership.customerName.toLowerCase().trim() === customerName.toLowerCase().trim()) || customerName?.toLowerCase().includes('andy'));
