...
import { Calendar, CheckCircle2, Clock3, QrCode, RefreshCw, Repeat, Trash2, X, ShieldCheck } from "lucide-react";
...
type LiveQr = { bookingId: string; qrValue: string; expiresAt: string; issuedAt: string; ttlSeconds: number };
...
export const ProductionMyBookingsScreen: React.FC<Props> = ({ onOpenBooking }) => {
  ...
  const [liveQr, setLiveQr] = useState<LiveQr | null>(null);
  ...
  const load = useCallback(async (silent = false) => {
    if (!silent) setLoading(true); else setRefreshing(true); setError(null);
    try { if (!localStorage.getItem("nexus_gaming_cafe_v1_accessToken")) throw new Error("Please sign in to load your live bookings."); const data = await fetchMyBookings({ limit: 100 }); setBookings(Array.isArray(data.bookings) ? data.bookings : []); }
    catch (e: any) { setError(e?.message || "Unable to load live bookings."); }
    finally { setLoading(false); setRefreshing(false); }
  }, []);

  const issueQr = useCallback(async (booking: Booking) => {
    setQrLoading(true); setQrError(null);
    try {
      const response = await fetch(`/api/bookings/${encodeURIComponent(booking.id)}/qr`, { method: "POST", headers: { ...authHeaders(), "Content-Type": "application/json" }, cache: "no-store" });
      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.error || "Unable to create QR pass");
      setLiveQr({ bookingId: booking.id, qrValue: data.qrValue, expiresAt: data.expiresAt, issuedAt: data.issuedAt, ttlSeconds: data.ttlSeconds });
    } catch (e: any) { setQrError(e?.message || "Unable to create QR pass"); }
  ...
  useEffect(() => { if (!selectedQr) return; void issueQr(selectedQr); const id = window.setInterval(() => void issueQr(selectedQr), 60000); return () => window.clearInterval(id); }, [selectedQr, issueQr]);
  ...
  const doCancel = async () => { if (!selectedCancel) return; try { const data = await cancelBookingOnServer(selectedCancel.id); setNotice(data.message || "Booking cancelled successfully."); setSelectedCancel(null); await load(true); } catch (e: any) { setError(e?.message || "Cancellation failed."); } };
  const doCheckIn = async (b: Booking) => { try { const data = await checkInBookingOnServer(b.id); setNotice(data.message || "Check-in successful."); await load(true); } catch (e: any) { setError(e?.message || "Check-in failed."); } };
  const doReschedule = async () => { if (!selectedReschedule || !newDate || !newTime) return; try { const data = await rescheduleBookingOnServer(selectedReschedule.id, { newDate, newStartTime: newTime, newSystemId: newSystemId || undefined }); setNotice(data.message || "Booking rescheduled successfully."); setSelectedReschedule(null); await load(true); } catch (e: any) { setError(e?.message || "Reschedule failed."); } };

  return <div className="flex flex-col gap-6 pb-12 animate-fadeIn select-none">
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-6"><div><span className="text-[10px] uppercase tracking-[0.4em] font-bold text-red-600">Live MongoDB Booking Feed</span><h1 className="text-3xl sm:text-4xl font-black text-white uppercase tracking-tight flex items-center gap-3 mt-1"><Calendar className="w-8 h-8" />My Bookings</h1><p className="text-sm text-white/50 mt-1">Server-authoritative reservations, live QR check-in and session status.</p></div><div className="flex gap-2"><button onClick={() => void load(true)} disabled={refreshing} className="px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-xs font-bold uppercase flex items-center gap-2 disabled:opacity-50"><RefreshCw className={`w-4 h-4 ${refreshing ? "animate-spin" : ""}`} />Sync</button><button onClick={() => onOpenBooking("Gaming PC")} className="px-5 py-3 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-black uppercase">Book New Station</button></div></div>
    ...
    {selectedQr && <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setSelectedQr(null)}><div className="w-full max-w-sm bg-[#0e0e0e] border border-red-500/20 rounded-3xl p-6 text-center" onClick={e => e.stopPropagation()}><div className="flex items-center justify-center gap-2 text-red-400 text-[10px] font-black uppercase tracking-[0.25em]"><ShieldCheck className="w-4 h-4" />Live secure pass</div>{qrLoading ? <div className="h-64 flex items-center justify-center text-white/50 text-sm"><RefreshCw className="w-5 h-5 animate-spin mr-2" />Generating live QR…</div> : qrError ? <div className="h-64 flex flex-col items-center justify-center text-red-400 text-sm px-4">{qrError}<button onClick={() => void issueQr(selectedQr)} className="mt-4 px-4 py-2 rounded-xl bg-white/10 text-white text-xs font-bold">Retry</button></div> : liveQr ? <><div className="mt-5 bg-white p-4 rounded-2xl inline-block"><QRCodeSVG value={liveQr.qrValue} size={240} level="M" includeMargin /></div><h3 className="mt-5 text-xl font-black text-white uppercase">{selectedQr.systemName}</h3><p className="text-xs text-white/50 font-mono mt-1">{selectedQr.id}</p><p className="text-sm text-white/70 mt-3">{selectedQr.date} · {selectedQr.startTime}–{selectedQr.endTime}</p><p className="text-[10px] text-amber-400 uppercase tracking-widest font-black mt-4">Rotates every 60 seconds · expires {Math.max(0, Math.ceil((new Date(liveQr.expiresAt).getTime() - clock) / 1000))}s</p><p className="text-[10px] text-white/35 mt-2">Show this QR to the front desk. Each pass can be used once.</p></> : null}<button onClick={() => setSelectedQr(null)} className="mt-6 w-full py-3 rounded-xl bg-white/10 text-white text-xs font-bold">Close</button></div></div>}
  </div>;
};
