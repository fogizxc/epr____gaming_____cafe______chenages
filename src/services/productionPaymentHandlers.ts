...
async function findCustomerBooking(db: any, bookingId: string, customerId: string) {
  return db.collection("bookings").findOne({ $or: [{ id: bookingId }, ...( /^[a-f0-9]{24}$/i.test(bookingId) ? [{ _id: new (await import("mongodb")).ObjectId(bookingId) }] : [])], customerId });
}
...
export async function handleProductionVerifyPayment(req: Request, res: Response) {
  ...
  try {
    ...
    const updated = await db.collection("payments").findOneAndUpdate({ _id: payment._id, status: { $ne: "CAPTURED" } }, { $set: { status: "CAPTURED", providerPaymentId: paymentId, providerSignatureVerifiedAt: new Date(), capturedAt: new Date(), updatedAt: new Date() } }, { returnDocument: "after" });
    ...
    console.error("payment/verify", error); return fail(res, 500, "Unable to verify payment");
  }
}
