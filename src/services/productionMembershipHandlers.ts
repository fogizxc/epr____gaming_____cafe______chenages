...
export async function handlePurchaseMembership(req: Request, res: Response) {
  ...
  const db = await getMongoDb(); const existing = await db.collection("customer_memberships").findOne({ customerId: u.id, purchaseIdempotencyKey: idempotencyKey }); if (existing) return res.json({ success: true, membership: safeMembership(existing), duplicate: true });
  const now = new Date(); const active = await db.collection("customer_memberships").findOne({ customerId: u.id, status: "ACTIVE", expiresAt: { $gt: now } }); if (active) return fail(res, 409, "An active membership already exists");
...
export async function handleActivateMembership(membershipId: string, paymentId: string, actorId: string, actorRole: string) {
  ...
  const membership = await db.collection("customer_memberships").findOneAndUpdate({ $or: membershipQuery(membershipId), status: "PENDING_PAYMENT", paymentStatus: "PENDING" }, { $set: { status: "ACTIVE", paymentStatus: "PAID", paymentId, startsAt: now, expiresAt, updatedAt: now } }, { returnDocument: "after" });
  if (!membership) return null;
  await writeAuditLog({ actorId, actorRole, action: "MEMBERSHIP_ACTIVATED", entityType: "customer_membership", entityId: membership.id, metadata: { paymentId, expiresAt: expiresAt.toISOString() } }); return membership;
}
...
export async function handleConsumeMembershipHours(req: Request, res: Response) {
  ...
  try { await session.withTransaction(async () => { const existing = await db.collection("wallet_transactions").findOne({ customerId: u.id, idempotencyKey }, { session }); if (existing) { updated = await db.collection("customer_memberships").findOne({ $or: membershipQuery(membershipId), customerId: u.id }, { session }); return; }
    ...
    updated = await db.collection("customer_memberships").findOneAndUpdate({ _id: membership._id, customerId: u.id, status: "ACTIVE", [field]: { $gte: hours } }, { $inc: { [field]: -hours }, $set: { updatedAt: now } }, { session, returnDocument: "after" }); if (!updated) throw new Error("MEMBERSHIP_USAGE_CONFLICT");
