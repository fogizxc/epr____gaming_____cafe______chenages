...
import { INITIAL_SYSTEMS } from "../data/initialData.js";
...
function pricingFor(station: any, rule: any, date: string, startTime: string): number {
  ...
  const peakStart = timeToMinutes(rule.peakHoursStart) ?? 18 * 60;
  const peakEnd = timeToMinutes(rule.peakHoursEnd) ?? 23 * 60;
  const isWeekend = day === "Saturday" || day === "Sunday";
  const isPeak = rule.isPeakEnabled !== false && (start >= peakStart && start < peakEnd);
  if (isWeekend && Number(rule.weekendPrice) > 0) return Number(rule.weekendPrice);
  if (isPeak && Number(rule.peakPrice) > 0) return Number(rule.peakPrice);
  return Number(rule.normalPrice) > 0 ? Number(rule.normalPrice) : fallback;
}

async function ensureStationSeeded() {
  const db = await getMongoDb();
  const count = await db.collection("gaming_systems").estimatedDocumentCount();
  if (count > 0) return;
  const now = new Date();
  const documents = INITIAL_SYSTEMS.map((s: any) => ({
    ...s,
    bookingVersion: 0,
...
export async function handleProductionAvailability(req: Request, res: Response) {
  try {
    ...
    if (!systemId || !validDate(date) || !Number.isFinite(duration) || duration < 0.5 || duration > 12 || duration % 0.5 !== 0) {
    ...
    }).project({ startTime: 1, endTime: 1 }).toArray();
    ...
    for (let start = OPEN_MINUTES; start + durationMinutes <= CLOSE_MINUTES; start += SLOT_MINUTES) {
      ...
      slots.push({
        ...
        available: !conflict && !past && !unavailable && station.status !== "ACTIVE",
        status: unavailable ? "MAINTENANCE" : past ? "PAST" : conflict ? "BOOKED" : station.status === "ACTIVE" ? "ACTIVE_SESSION" : "AVAILABLE",
        reason: unavailable ? "Station is unavailable" : past ? "Time slot has passed" : conflict ? "Reserved by another customer" : station.status === "ACTIVE" ? "Station is currently occupied" : undefined,
      });
    }
...
export async function handleProductionCreateBooking(req: Request, res: Response) {
  ...
  try {
    ...
    if (!systemId || !validDate(date) || !/^([01]\d|2[0-3]):[0-5]\d$/.test(String(startTime)) || !Number.isFinite(duration) || duration < 0.5 || duration > 12 || duration % 0.5 !== 0) {
      return res.status(400).json({ success: false, error: "Invalid booking parameters" });
    }
    if (!idempotencyKey || idempotencyKey.length < 16 || idempotencyKey.length > 128) {
      return res.status(400).json({ success: false, error: "A valid Idempotency-Key is required" });
    }

    const start = timeToMinutes(String(startTime))!;
    const end = start + Math.round(duration * 60);
    if (start < OPEN_MINUTES || end > CLOSE_MINUTES || start % SLOT_MINUTES !== 0) {
      return res.status(400).json({ success: false, error: "Booking is outside operating hours" });
    }
    ...
    const result = await withTransaction(async (db, session) => {
      ...
      const station = await systems.findOneAndUpdate(
        ...
        { $inc: { bookingVersion: 1 }, $set: { updatedAt: new Date() } },
      ...
      const gamingSubtotalPaise = Math.round(hourlyRate * duration * 100);
      const bookingId = `BK-${new Date().getTime()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
      const now = new Date();
      const user = await db.collection("users").findOne({ _id: station._id as any });
      void user;
      const customer = await db.collection("users").findOne({ _id: authReq.user!.id.length === 24 ? new (await import("mongodb")).ObjectId(authReq.user!.id) : undefined } as any, { session });
      const customerName = customer?.name || authReq.user!.name || "Customer";
      ...
      const booking = {
        ...
        applicableRatePaise: Math.round(hourlyRate * 100),
        ...
        foodTotal: 0,
        foodTotalPaise: 0,
        subtotalPaise: gamingSubtotalPaise,
        finalAmount: gamingSubtotalPaise / 100,
        finalAmountPaise: gamingSubtotalPaise,
    ...
    if (!result.duplicate) {
      await writeAuditLog({ actorId: authReq.user!.id, actorRole: authReq.user!.role, action: "BOOKING_CREATED", entityType: "booking", entityId: result.existing.id, metadata: { systemId, date, startTime, duration } });
    }
