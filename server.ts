...
import { registerCatalogRoutes } from "./src/server/catalogBootstrap.js";
...
async function startServer() {
  ...
  app.use(express.json({ limit: process.env.JSON_BODY_LIMIT || "1mb", verify: (req: any, _res, buf) => { req.rawBody = Buffer.from(buf); } }));
  ...
  app.post("/api/brevo/send", async (req, res) => {
    try { const payload = req.body as BrevoEmailPayload; if (!payload?.to || !/^([^\s@]+)@([^\s@]+)\.([^\s@]+)$/.test(payload.to)) return res.status(400).json({ success: false, error: "Valid recipient email address is required" }); if (!payload.subject || typeof payload.subject !== "string") return res.status(400).json({ success: false, error: "Subject is required" }); const config = getBrevoConfig(); if (!config.isConfigured) return res.status(503).json({ success: false, error: "Brevo is not configured. Set BREVO_SMTP_USER, BREVO_SMTP_KEY and BREVO_FROM_EMAIL." }); let messageId: string | null = null; let deliveryMethod = "Brevo SMTP Relay"; try { const transporter = createBrevoTransporter(); if (!transporter) throw new Error("Brevo transporter unavailable"); const info = await transporter.sendMail({ from: `"${config.fromName}" <${config.fromEmail}>`, to: payload.toName ? `"${payload.toName}" <${payload.to}>` : payload.to, subject: payload.subject, text: payload.textContent, html: generateEmailHtml(payload) }); messageId = info.messageId; } catch { const result = await sendViaBrevoApi(payload, config); deliveryMethod = "Brevo Transactional API"; messageId = (result as { messageId?: string })?.messageId || "api-dispatched"; } res.json({ success: true, recipient: payload.to, deliveryMethod, messageId, timestamp: new Date().toISOString() }); }
    catch (err: any) { res.status(500).json({ success: false, error: err.message || "Brevo delivery failed" }); }
  });
  app.post("/api/brevo/test", async (req, res) => { const targetEmail = typeof req.body?.testEmail === "string" ? req.body.testEmail : getBrevoConfig().fromEmail; if (!targetEmail) return res.status(400).json({ success: false, error: "Provide testEmail or configure BREVO_FROM_EMAIL." }); try { const config = getBrevoConfig(); if (!config.isConfigured) return res.status(503).json({ success: false, error: "Brevo is not configured." }); const result = await sendViaBrevoApi({ to: targetEmail, subject: "Brevo Integration Test • Bytes & Brew Gaming Café", type: "CUSTOM" }, config); res.json({ success: true, message: `Test email successfully sent to ${targetEmail} via Brevo!`, result }); } catch (err: any) { res.status(500).json({ success: false, error: err.message || "Failed to send test email" }); } });
  ...
  const shutdown = async (signal: string) => { console.log(`[server] ${signal} received; starting graceful shutdown`); server.close(async () => { try { await closeMongoDb(); } catch (error) { console.error("[server] MongoDB shutdown error:", error); } process.exit(0); }); setTimeout(() => process.exit(1), 10_000).unref(); };
  process.once("SIGTERM", () => void shutdown("SIGTERM")); process.once("SIGINT", () => void shutdown("SIGINT")); process.on("unhandledRejection", (reason) => console.error("[server] unhandledRejection", reason)); process.on("uncaughtException", (error) => { console.error("[server] uncaughtException", error); void shutdown("uncaughtException"); });
}
